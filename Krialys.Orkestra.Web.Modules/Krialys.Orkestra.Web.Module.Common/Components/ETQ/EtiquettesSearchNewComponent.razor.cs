...
public partial class EtiquettesSearchNewComponent
{
    [Parameter] public EventCallback<string> OnSearchCriteriaChanged { get; set; }
    [Parameter] public string SearchODataFilter { get; set; }
    [Parameter] public string KeyCache { get; set; }
    [Parameter] public EventCallback<string> SearchODataFilterChanged { get; set; }
    [Parameter] public bool OnlyAllowedPerimeters { get; set; }

    [Parameter]
    public ResearchType[] ResearchType { get; set; } =
        new ResearchType[]
        {
            ETQ.ResearchType.BySimpleText,
            ETQ.ResearchType.ByDate,
            ETQ.ResearchType.ByDomaine,
            ETQ.ResearchType.ByPerimeter,
            ETQ.ResearchType.ByLabelOject
        };

    public async Task OnMajeurVersionChanged(Microsoft.AspNetCore.Components.ChangeEventArgs e)
    {
        SearchODataFilter = e.Value.ToString();
        await SearchODataFilterChanged.InvokeAsync(SearchODataFilter);
    }

    private bool SortByNewest;

    private void OnSortChanged()
    {
        SearchODataFilter = SortByNewest ?
            $"$orderby={nameof(TETQ_ETIQUETTES.TETQ_DATE_CREATION)} desc"
            : $"$orderby={nameof(TETQ_ETIQUETTES.TETQ_CODE)} asc";

        // Trigger event callback.
        //await OnSearchCriteriaChanged.InvokeAsync(SearchODataFilter);
    }
    ...
    private readonly Query _ruleValueQuery = new Query()
        .Sort(nameof(TRGLRV_REGLES_VALEURS.TRGLRV_VALEUR), Syncfusion.Blazor.Data.SortOrder.Ascending.ToString())
        .Sort($"{nameof(TRGLRV_REGLES_VALEURS.TRGL_REGLE)}.{nameof(TRGL_REGLES.TRGL_LIB_REGLE)}", Syncfusion.Blazor.Data.SortOrder.Ascending.ToString())
        .AddParams(Litterals.OdataQueryParameters, $"?$expand={nameof(TRGLRV_REGLES_VALEURS.TRGL_REGLE)}")
        .Take(20);

    private readonly Query _actionDropDownQuery = new Query()
        .Sort(nameof(TACT_ACTIONS.TACT_LIB), Syncfusion.Blazor.Data.SortOrder.Ascending.ToString());
    ...
    private class EtqFilters
    {
        ...
        public TRGLRV_REGLES_VALEURS[] _selectedRuleValues;
    ...
    private async Task UpdateEtqFilteringAsync()
    {
        ...
        // Add rules filters.
        if (_filters._selectedRuleValues != null && _filters._selectedRuleValues.Any())
        {
            //filtersTable.Add(string.Join(" or ", _filters.SelectedDomains.Select(d =>
            //        $"TOBJE_OBJET_ETIQUETTE/TDOM_DOMAINEID eq {d.TDOM_DOMAINEID}")));
        }

        if (!string.IsNullOrEmpty(_filters.ActionLabel))
        {
            //filtersTable.Add(string.Join(" or ", _filters.SelectedDomains.Select(d =>
            //        $"TOBJE_OBJET_ETIQUETTE/TDOM_DOMAINEID eq {d.TDOM_DOMAINEID}")));
        }
        ...
        _odataFilter =
            SortByNewest ? $"$orderby={nameof(TETQ_ETIQUETTES.TETQ_DATE_CREATION)} desc" : $"orderby={nameof(TETQ_ETIQUETTES.TETQ_CODE)} asc"
            + (string.IsNullOrEmpty(_odataFilter) ? "" : "&" + _odataFilter);
    ...
    /// </summary>
    /// <param name="args">Select event arguments.</param>
    private Task PerimeterFilterOnValueSelectAsync(SelectEventArgs<TPRCP_PRC_PERIMETRES> args)
    {
        // Update selected perimeters.
        _filters.SelectedPerimeters.Add(args.ItemData);

        return UpdateEtqFilteringAsync();
    }
    ...
    private Task PerimeterFilterValueRemovedAsync(RemoveEventArgs<TPRCP_PRC_PERIMETRES> args)
    {
        // Remove chips.
        // When chips depend on a list, they are not automatically removed when the list is updated.
        if (_perimeterSfChipRef != null)
    ...
    private Task PerimeterFilterClearedAsync(MouseEventArgs args)
    {
        // Update selected perimeters.
        _filters.SelectedPerimeters.Clear();
    ...
    /// </summary>
    /// <param name="args">Select event arguments.</param>
    private Task DomainFilterOnValueSelectAsync(SelectEventArgs<TDOM_DOMAINES> args)
    {
    ...
    /// </summary>
    /// <param name="args">Remove event arguments.</param>
    private Task DomainFilterValueRemovedAsync(RemoveEventArgs<TDOM_DOMAINES> args)
    {
        // Remove chips.
        // When chips depend on a list, they are not automatically removed when the list is updated.
        if (_domainSfChipRef != null)
    ...
    /// </summary>
    /// <param name="args">Mouse event arguments.</param>
    private Task DomainFilterClearedAsync(MouseEventArgs args)
    {
        // Update selected domains.
        _filters.SelectedDomains.Clear();
    ...
    private readonly Query _moduleQuery = new Query()
        .Sort(nameof(TS_SCENARIOS.TS_NOM_SCENARIO), Syncfusion.Blazor.DropDowns.SortOrder.Ascending.ToString())
        .Sort($"{nameof(TS_SCENARIOS.TE_ETAT)}.{nameof(TE_ETATS.TE_FULLNAME)}", Syncfusion.Blazor.DropDowns.SortOrder.Ascending.ToString())
    ...
    /// </summary>
    /// <param name="args">Select event arguments.</param>
    private Task ObjectFilterOnValueSelectAsync(SelectEventArgs<TOBJE_OBJET_ETIQUETTES> args)
    {
    ...
    /// </summary>
    /// <param name="args">Remove event arguments.</param>
    private Task ObjectFilterValueRemovedAsync(RemoveEventArgs<TOBJE_OBJET_ETIQUETTES> args)
    {
        // Remove chips.
        // When chips depend on a list, they are not automatically removed when the list is updated.
        if (_objectSfChipRef != null)
    ...
    /// </summary>
    /// <param name="args">Mouse event arguments.</param>
    private Task ObjectFilterClearedAsync(MouseEventArgs args)
    {
        // Update selected "labeled objects".
        _filters.SelectedObjects.Clear();
