...
internal class SQLiteSink : BatchProvider, ILogEventSink
{
    ...
    {
        var colDefs = "log_id INTEGER PRIMARY KEY AUTOINCREMENT,";
        colDefs += "log_creation_date TEXT,";
        colDefs += "log_type VARCHAR(10),";
        colDefs += "log_exception TEXT,";
        colDefs += "log_message TEXT,";
        colDefs += "log_message_details TEXT";
    ...
    {
        var sqlInsertText = "INSERT INTO {0} (log_creation_date, log_type, log_exception, log_message, log_message_details)";
        sqlInsertText += " VALUES (@log_creation_date, @log_type, @log_exception, @log_message, @log_message_details)";
        ...
        sqlCommand.Parameters.Add(new SQLiteParameter("@log_creation_date", DbType.DateTime2));
        sqlCommand.Parameters.Add(new SQLiteParameter("@log_type", DbType.String));
        sqlCommand.Parameters.Add(new SQLiteParameter("@log_exception", DbType.String));
        sqlCommand.Parameters.Add(new SQLiteParameter("@log_message", DbType.String));
        sqlCommand.Parameters.Add(new SQLiteParameter("@log_message_details", DbType.String));
    ...
    {
        var cmd = sqlConnection.CreateCommand();
        cmd.CommandText = $"DELETE FROM {_tableName} WHERE log_creation_date < @epoch";
        cmd.Parameters.Add(
    ...
    protected override async Task<bool> WriteLogEventAsync(ICollection<LogEvent> logEventsBatch)
    {
        ...
        try
        {
            ...
            try
            {
                await WriteToDatabaseAsync(logEventsBatch, sqlConnection);
                return true;
            }
    ...
    private async Task WriteToDatabaseAsync(ICollection<LogEvent> logEventsBatch, SQLiteConnection sqlConnection)
    {
        ...
        {
            sqlCommand.Parameters["@log_creation_date"].Value = _storeTimestampInUtc
                ? logEvent.Timestamp.ToUniversalTime().ToString("yyyy-MM-dd HH:mm:ss")
                : logEvent.Timestamp.ToString("yyyy-MM-dd HH:mm:ss");
            sqlCommand.Parameters["@log_type"].Value = logEvent.Level.ToString();
            sqlCommand.Parameters["@log_exception"].Value =
                logEvent.Exception?.ToString() ?? string.Empty;
            sqlCommand.Parameters["@log_message"].Value = logEvent.MessageTemplate.ToString();

            sqlCommand.Parameters["@log_message_details"].Value = logEvent.Properties.Count > 0
