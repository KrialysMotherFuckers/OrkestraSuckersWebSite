...
public class JwtAuthenticationService : AuthenticationStateProvider, IAuthenticationService, IAccessTokenProvider
{
    ...
    public override async Task<AuthenticationState> GetAuthenticationStateAsync()
    {
        ...
        {
            claimsIdentity.AddClaims(cachedPermissions.Select(p => new Claim(OrkaClaims.Permission, p)));
        }
    ...
    {
        var tokenResponse = await _tokensClient.GetTokenAsync(request); // GetTokenAsync(tenantId, request);
    ...
    {
        var authState = await GetAuthenticationStateAsync();
        var tenantKey = authState.User.GetTenant();
        ...
        try
        {
            var tokenResponse = await _tokensClient.RefreshAsync(request); // (tenantKey, request);
    ...
    {
        await _localStorage.SetItemAsync(Litterals.AuthToken, token);
        await _localStorage.SetItemAsync(Litterals.RefreshToken, refreshToken);
    }

    private ValueTask CachePermissions(ICollection<string> permissions) =>
        _localStorage.SetItemAsync(Litterals.Permissions, permissions);

    private async Task ClearCacheAsync()
    {
        await _localStorage.RemoveItemAsync(Litterals.AuthToken);
        await _localStorage.RemoveItemAsync(Litterals.RefreshToken);
        await _localStorage.RemoveItemAsync(Litterals.Permissions);
    }

    private ValueTask<string> GetCachedAuthTokenAsync() =>
        _localStorage.GetItemAsync<string>(Litterals.AuthToken);

    private ValueTask<string> GetCachedRefreshTokenAsync() =>
        _localStorage.GetItemAsync<string>(Litterals.RefreshToken);

    private ValueTask<ICollection<string>> GetCachedPermissionsAsync() =>
        _localStorage.GetItemAsync<ICollection<string>>(Litterals.Permissions);
