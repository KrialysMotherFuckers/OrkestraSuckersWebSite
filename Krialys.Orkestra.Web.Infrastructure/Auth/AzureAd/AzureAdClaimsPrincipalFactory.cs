...
namespace Krialys.Orkestra.Web.Infrastructure.Auth.AzureAd;
...
internal class AzureAdClaimsPrincipalFactory : AccountClaimsPrincipalFactory<RemoteUserAccount>
{
    ...
    public override async ValueTask<ClaimsPrincipal> CreateUserAsync(RemoteUserAccount account, RemoteAuthenticationUserOptions options)
    {
        ...
        if (principal.Identity?.IsAuthenticated is true)
        {
            ...
            if (!userIdentity.HasClaim(c => c.Type == OrkaClaims.Fullname))
            {
                userIdentity.AddClaim(new Claim(OrkaClaims.Fullname, $"{userDetails.FirstName} {userDetails.LastName}"));
            }
            ...
            if (!string.IsNullOrWhiteSpace(userDetails.ImageUrl) && !userIdentity.HasClaim(c => c.Type == OrkaClaims.ImageUrl) && userDetails.ImageUrl is not null)
            {
                userIdentity.AddClaim(new Claim(OrkaClaims.ImageUrl, userDetails.ImageUrl));
            }

            var permissions = await _services.GetRequiredService<IPersonalClient>().GetPermissionsAsync();

            userIdentity.AddClaims(permissions.Select(permission => new Claim(OrkaClaims.Permission, permission)));
        }
